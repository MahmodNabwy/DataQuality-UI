...
export default function IssuesReport({
  ...
  const isIssueResolved = (issueId?: string): boolean => {
    if (!issueId) return false;
    const status = getIssueStatus(issueId);
    return locallyResolved.has(issueId) || status?.status === "resolved";
  };

  const handleMarkResolved = (issueId?: string) => {
    if (!issueId) return;

    // Add to locally resolved set
  ...
  const handleUndoResolved = (issueId?: string) => {
    if (!issueId) return;

    // Remove from locally resolved set
    ...
    // You might want to call a parent handler for undo if needed
    // if (onIssueStatusChange) {
    //   onIssueStatusChange(issueId, "active");
    // }
  };
  ...
  const severityColor = (severity: string) => {
    switch (severity) {
      ...
      case "warning":
        return "border-[#0986ed]/30 bg-[#053964]/95 backdrop-blur";
      case "info":
        return "border-[#0986ed]/30 bg-[#053964]/95 backdrop-blur";
      default:
  ...
  const resolvedCount = issueStatuses.filter(
    (s) => s.status === "resolved"
  ).length;
  const dismissedCount = issueStatuses.filter(
  ...
  return (
    <div className="space-y-8">
            ...
            {totalOriginalIssues > 0 && (
              <div className="mt-4">
                <div className="flex justify-between text-sm text-blue-300 mb-2">
                  ...
                  <span>
                    {Math.round(
                      ((resolvedCount + dismissedCount) / totalOriginalIssues) *
                        100
                    )}
                    %
                  </span>
            ...
            <div className="grid gap-3">
              {issues.map((issue, idx) => {
                ...
                return (
                  <Card
                    key={idx}
                    className={`border transition-all hover:shadow-md ${severityColor(
                      issue.severity
                    )}`}
                  >
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-start gap-4">
                          <div className="flex-1">
                            <p className="font-semibold text-2xl text-blue-100">
                              {issue.indicatorName}
                            </p>
                            {issue.filterName && (
                          ...
                          <div className="flex items-center gap-2">
                            <Badge
                              variant="outline"
                              className={`text-sm ${severityTextColor(
                                issue.severity
                              )}`}
                            >
                        ...
                        {issue.details && (
                          <div className="mt-3 p-3 bg-black/40 rounded border border-blue-800/50 text-xl font-mono text-blue-300 overflow-x-auto">
                        ...
                        {onIssueStatusChange && (
                          <div className="flex gap-2 mt-4 pt-3 border-t border-blue-800/30">
                            {isIssueResolved(issue.id) ? (
                              <div className="flex items-center gap-2">
                                <Button
                                  onClick={() => handleUndoResolved(issue.id)}
                                  size="lg"
                                  variant="outline"
                                  className="gap-2 border-yellow-600/60 text-yellow-300 hover:bg-yellow-800/20 hover:text-yellow-200"
                                >
                              ...
                              <>
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  variant="outline"
                                  className="gap-2 cursor-pointer border-gray-600 text-gray-300 hover:bg-red-800 hover:text-white"
                                >
                                  <XCircle className="w-4 h-4" />
                                  إلغاء المشكلة
                                </Button>
      ...
      <Dialog
        ...
        onOpenChange={(open) => setDismissDialog({ open })}
      >
        <DialogContent className="bg-slate-900 border border-blue-800/40 shadow-xl rounded-xl">
          <DialogHeader className="pb-3 border-b border-blue-600/30">
            <DialogTitle className="text-blue-100 text-lg font-semibold tracking-wide">
              إلغاء المشكلة
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-5 py-5">
            <div className="space-y-2">
              ...
              >
                رقم الجدول <span className="text-blue-100">*</span>
              </Label>
              <Input
                id="tableNumber"
                value={tableNumber}
                ...
                className="bg-slate-800/60 border border-blue-700/40 focus:border-white focus:ring-blue-500 text-blue-100 rounded-md transition-all"
                required
              />
            </div>

            <div className="space-y-2">
              <Label htmlFor="comment" className="text-blue-100 font-medium">
                التعليق <span className="text-blue-100">*</span>
              </Label>
              <Textarea
                id="comment"
                value={comment}
                ...
                className="bg-slate-800/60 border border-blue-700/40 focus:border-white focus:ring-blue-500 text-blue-100 min-h-[110px] rounded-md transition-all"
                required
              />
            </div>
          </div>

          <DialogFooter className="border-t border-blue-800/30 pt-4">
            <Button
              variant="outline"
              className="border-blue-700/40 text-blue-200 hover:bg-white cursor-pointer transition"
            ...
            <Button
              ...
              className="bg-blue-600 hover:bg-blue-700 cursor-pointer text-blue-50 shadow-md shadow-blue-900/20 transition"
            >
              تأكيد الإلغاء
            </Button>
          </DialogFooter>
